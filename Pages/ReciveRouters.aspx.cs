...
{
    public partial class ReciveRouters : CustomPage
    {
        ...
        void PopulateGrid()
        {
            ...
            {
                var contex2 = new Db.ISPDataContext(ConfigurationManager.AppSettings["ConnectionString"]);
                ...
                {
                    var orderexist = CheckIfAlreadyExist(order.ID, contex2);
            ...
            using (var context = new Db.ISPDataContext(ConfigurationManager.AppSettings["ConnectionString"]))
            {
                ...
                else
                {
                    ...
                    var receivefromcompany = new Db.RecieveRouter
                    {
                        ...
                        StoreId = storeId,
                        //IsRecieved = false,
                        WorkOrderIdRecive = Convert.ToInt32(Hfworderrouter.Value),
        ...
        protected Db.RecieveRouter CheckIfAlreadyExist(int id, Db.ISPDataContext context)
        {
        ...
        private void ProcessQuery()
        {
            using (var context = new Db.ISPDataContext(ConfigurationManager.AppSettings["ConnectionString"]))
            {
                ...
                var newOrderList = new List<Db.WorkOrder>();
                foreach (var order in orderList)
                {
